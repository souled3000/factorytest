...
import com.vision.factorytest.utils.*;
...
public class ReceiveMessageManager {
	...
	public static ReceiveMessageManager defaultManage() {
		return receiveMessageManager == null ? receiveMessageManager = new ReceiveMessageManager()
	...
	public void SerialPortDataResponse(byte[] serialPortData) {
		LogUtils.d("串口返回数据", ByteUtils.byteArrayToHexString(serialPortData));
	...
	private void deviceReset(BaseFrame frame, String chip, String mac, SerialPort serialPort, byte[] serialPortData,
			String str) {
		LogUtils.d("模块恢复出厂", str);
		if (DeviceConstant.Chip._7681.equals(chip)) {
			...
			} else if (str.contains(DeviceConstant.RMessage_7681.REC_ENTER)
					|| str.contains(DeviceConstant.RMessage_7681.REC_INTO)
					|| str.contains(DeviceConstant.RMessage_7681.AUTH_OK)) {
				frame.clearDataView();
			...
			} else if ((str.contains(DeviceConstant.RMessage_7688.REC_ENTER)
					|| str.contains(DeviceConstant.RMessage_7688.REC_INTO)) && !isRec) {
				frame.clearDataView();
				frame.setDataView("进入模块恢复出厂模式成功");
				frame.setDataView("发送恢复出厂设置指令");
				frame.setDataView("等待恢复出厂");
	...
	public void HttpResponse(String content, String urlOrigin) {
		LogUtils.d("http接收：", "urlOrigin:" + urlOrigin + "___content:" + content);
		...
		if (Constant.UrlOrigin.upload_test_result.equals(urlOrigin)) { // 上传测试结果
			...
			LogUtils.d("获取测试结果", "chip:" + chip + "___isSuccess:" + isSuccess);
	 ...
	 *            数据(字符串)
	 */
	private void writeFactoryDistrict(BaseFrame frame, String chip, String mac, final SerialPort serialPort,
			byte[] data, String str) {
		LogUtils.d("写模块工厂区", str);
		...
		if (DeviceConstant.Chip._2630.equals(chip)) {
			if (DeviceConstant.RMessage_2630.POWER_ON.equals(str) && !isReboot) {
				...
				sendMessageManager.sendOrder(serialPort, DeviceConstant.Order_2630.REBOOT);

			} else if (DeviceConstant.RMessage_2630.REBOOT.equals(str)) {
				isReboot = true;
				...
				timer.schedule(new TimerTask() {
					...
					|| ByteUtils.byteArrayToHexString(data).contains(mac.substring(12))) {
				// 获取2630设备信息，有时数据分开返回，特殊处理
				String deviceInfo = ByteUtils.byteArrayToHexString(data);
				if (deviceInfo.length() != 38) {
					builder.append(deviceInfo);
					if (deviceInfo.contains("6D64")) {
						return;
					} else if (deviceInfo.contains(mac.substring(12))) {
						data = ByteUtils.hexStr2Byte(builder.toString());
						LogUtils.d("获取设备信息", builder.toString());
					}
				}
			...
			if (str.contains(DeviceConstant.RMessage_7681.POWER_ON) && !isReboot) {
				LogUtils.d("7681", "进入写模块工厂区模式");
				frame.clearDataView();
				sendMessageManager.sendOrder(serialPort, DeviceConstant.Order_7681.REC);

			} else if ((str.contains(DeviceConstant.RMessage_7681.REC_ENTER)
					|| str.contains(DeviceConstant.RMessage_7681.REC_INTO)) && !isRec) {
				frame.setDataView("进入写模块工厂区模式成功");
				...
				LogUtils.d("7681", "开始写入License文件");
				sendMessageManager.sendData_768(serialPort, DeviceConstant.Chip._7681, CommUtils.buildLicensePath(mac));
				isLicenseReady = false;

			} else if ((str.contains(DeviceConstant.RMessage_7681.WRITE_LICENSE_OK)
					|| str.contains(DeviceConstant.RMessage_7681._WRITE_LICENSE_OK)) && !isWriteLicenseOk && isRec) {
	 ...
	 *            数据(字符串)
	 */
	private void RSSITest(final BaseFrame frame, final String chip, final String mac, final SerialPort serialPort,
			byte[] data, String str) {
		LogUtils.d("通讯质量测试", str);

		if (DeviceConstant.Chip._2530.equals(chip) || DeviceConstant.Chip._2630.equals(chip)) {
			LogUtils.d("ZigBee通讯质量测试", ByteUtils.byteArrayToHexString(data));
			...
			if ((str.contains(DeviceConstant.RMessage_7681.REC_ENTER)
					|| str.contains(DeviceConstant.RMessage_7681.REC_INTO)) && !isRec) {
				frame.clearDataView();
				frame.setDataView("进入通讯质量测试模式成功\r\n正在连接WiFi...");
	 ...
	 *            数据(字符串)
	 */
	private void deviceUpgrade(final BaseFrame frame, final String chip, final String mac, final SerialPort serialPort,
			byte[] data, String str) {
		LogUtils.d("模块程序升级", str);
		...
		if (DeviceConstant.Chip._2530.equals(chip)) {
			...
			if (str.contains(DeviceConstant.RMessage_7681.POWER_ON)) {
				 ...
				 * else if (str.contains(DeviceConstant.RMessage_7681.AUTH_OK) && isReboot) {
				 * frame.setDataView("发送恢复出厂设置指令"); frame.setDataView("等待恢复出厂"); isReboot =
				 * false; isResetDevice=true; sendMessageManager.sendOrder(serialPort,
				 * DeviceConstant.Order_7681.RESET_DEVICE);
				 * 
				 * }else if(str.contains(DeviceConstant.RMessage_7681.AUTH_OK) &&
				 * isResetDevice){ isResetDevice=false; frame.setDataView("\r\n恢复出厂成功");
				 * SerialPortManager.isRefuseReadData = true;
				 * 
				 * sendMessageManager.uploadTestResult(mac, true, 3, chip); }
				 */
			...
			/*
			 * if (str.contains(DeviceConstant.RMessage_7688.POWER_ON)) {
			 * frame.clearDataView(); sendMessageManager.sendOrder(serialPort,
			 * DeviceConstant.Order_7688.REC);
			 * 
			 * } else if ((str.contains(DeviceConstant.RMessage_7688.REC_ENTER) || str
			 * .contains(DeviceConstant.RMessage_7688.REC_INTO)) && !isRec) {
			 * frame.setDataView("进入模块恢复出厂模式成功"); frame.setDataView("发送恢复出厂设置指令");
			 * frame.setDataView("等待恢复出厂"); sendMessageManager.sendOrder(serialPort,
			 * DeviceConstant.Order_7688.RESET_DEVICE); isRec = true; isResetDevice=true;
			 * }else if(str.contains(DeviceConstant.RMessage_7688.AUTH_OK) &&
			 * isResetDevice){ isResetDevice=false; isRec = false;
			 * frame.setDataView("\r\n恢复出厂成功"); sendMessageManager.uploadTestResult(mac,
			 * true, 4, chip); }
			 */
		}
	}
